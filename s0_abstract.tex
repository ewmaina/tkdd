...
Graphs have become increasingly important structures for representing and understanding complex structures. Enumeration of subgraphs requires first deciding whether a graph is a subgraph of another graph. This is called the subgraph isomorphism problem and is NP complete. In this study we are concerned with the efficient enumeration of all subgraphs of a query isomorphic to the graphs in a database. 

2. DAG data structure
First, we construct a database by decomposing database graphs and storing all the subgraphs in the nodes of a Directed Acyclic Graph (DAG).
...
We implement these heuristics as a traversal of the DAG data structure. 
