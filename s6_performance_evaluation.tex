...
The source code was developed in the C++ language and compiled using Microsoft Visual Studio 2010 Ultimate. The hardware consisted of a Dell T1500 personal computer with a Intel Core i-3 CPU, 3.09 GHZ, 16 Gbyte memory, running the Windows 7 Professional Operating system.
...
\texttt{(a)} Linear and \texttt{(b)} Linear-log plot of \textit{induced subgraph} query processing time against graph density. \texttt{(c)} Linear and \texttt{(d)} Linear-log plot of \textit{subgraph} query processing time for increasing in graph density.
}
\label{fig:fig91}
\end{figure}
