...
\cite{bunke_glauser_tran1991} developed the their RETE inspired graph matching algorithm to allow match of dynamically changing scene graphs. To do so the graphs are decomposed by deleting edges to a (possibly very) large number of prototype graphs which are stored in the DAG. The links  DAG network are constructed to represent addition of edges in the dynamic graph.



In \cite{messmer_bunke2000} the DAG is build offline by random decomposition of the database graphs. As a result the structure of the DAG network is not unique and varies randomly.  
