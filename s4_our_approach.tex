We describe briefly the database creation and the search processes of  \textit{Network Method} then we detail our contributions in terms for developments in the data structure as well as a new fast graph search strategy. 

Specifically,  this work we use a modified version of the $DAG$ data structure used in the \textit{Network Method}. We describe in detail this data structure, the modifications that we have made in the \textit{Network Method}. We also implement a new graph search algorithm on the $DAG$ for subgraph isomorphism search. We also implement a search algoritm or induced subgraph. This allows us to compare the performance of our Method to the   
