...
Messmer et al.\cite{messmer_bunke2000} proposed a method to search graphs which are induced subgraphs of a query graph from a graph database. Central to the method is a $DAG$ data structure that is used to store decomposed subgraphs of a graph database in a compact form. Messmer called this data structure a network so here we refer to their method as the \textit{Network Method}. 
