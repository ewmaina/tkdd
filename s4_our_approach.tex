...
We have two main contributions. We propose a new search strategy for the database. We use this algorithm to implement fast and scalable graph database creation and query processing. Secondly, propose modifications to the $DAG$ data structure. By implementing these proposals we subgraph query search and subgraph query search on the modified $DAG$. that implements improvements in the core data structure to enable processing of both subgraphs and induced subgraphs, and propose a new fast and scalable search strategy for the database.
We propose improvements in two areas: extension of processing to subgraph isomorphism query and more scalable \textit{Network Method}. First, We describe below the improvements to the \textit{Network Method} aimed at more efficient computation of induced subgraph query in the context of induced subgraph query processing. Then we describe the new extensions that enable subgraph query processing. 
