...
\caption{\textbf{Results of the Experiments on the Synthetic Graph Data Set:} \textbf{ Top left;}Linear-linear plot of  induced subgraph query processing time with increasing size of query graph.The smallest query graph contains 10 vertices while the largest query graph contains 800 vertices. The \textit{Fast Network Method} and the textit{Network Method} show the fastest processing time for induced subgraph queries of less that 600 vertices. For larger query graphs, the \textit{VF2} method is faster. The \textit{Network Method} is the slowest for this dataset, taking more than 5sec for any induced subgraph query graph greater of 10 vertices or more. The \textit{VF2} method shows an almost linear time dependency, which is advantageous for induced subgrapg queries greater than 600 vertices. 
