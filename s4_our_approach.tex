...
We now define a decomposition method by partitioning edges, that is suited for processing subgraphs on the $DAG$.
...
Hence for subgraph query processing we decompose the graphs by partitioning edges and include the corresponding vertices.
...
We reformulate the decomposition algorithm with the restriction that a connected graph must be decomposed to strictly result in only two connected subgraphs. 
