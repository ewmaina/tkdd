...
one model graph in a graph database that must be matched with the input graph. Consequently it is necessary to apply the subgraph isomorphism algorithm to each input pair 
...
A newer approach to improving search over multiple graphs is construct an efficient representation of database graphs. One such representation  

We can classify a third, more recent and Approach aims to reduce the cost of matching a query graph against a very large library of model graphs.
One example of this approach,  was proposed by Messmer and Bunke \cite{messmer_bunke2000}. Their work is based on the RETE match algorithm for initially conceived production matching in expert systems.
