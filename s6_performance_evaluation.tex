...
To test the performance of our algorithm with regard to induced subgraph query, we evaluate the proposed algorithm in comparison with Messmer's Network Algorithm and a sequential SCAN using the state-of-the-art subgraph isomorphism detection algorithm VF2\cite{cordella2001_vf2}.
We also evaluate our algorithm with regard to non-induced subgraph query, where we compare proposed algorithm with the sequencial scan(VF2) only.


\subsubsection{Influence of size of query and Database graph on processing time}

\subsubsection{Influence of Number of Labels in Query and Database Graphs on processing time}
...
$D$ is composed of 18930 distict frequent subgraphs.
