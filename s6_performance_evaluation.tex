...
\textt{(a):} Linear plot of induced induced subgraph query processing time against query graph vertex count. 

\textt{(b):} Linear-log plot of the induced subgraph graph query processing time with query graph size. 

\textt{(c):} Bottom left: Linear-linear plot of  subgraph query processing time for increasing size of graph query.

\textt{(d):} Bottom Right Linear-log plot of the  subgraph graph query processing time with query graph size. 
}
\label{fig:fig91}
\end{figure}
