...
Messmer et al.\cite{messmer_bunke2000} proposed a method to search graphs which are induced subgraphs of a query graph from a graph database. Central to the method is a $DAG$ data structure used to store decomposed subgraphs in a compact form. Messmer called this datastructure a network so here we refer to their method as the \textit{network method}. 
