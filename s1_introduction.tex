...
query graphs.We consider the details of the algorithms in the following section.


We introduce a new term that relates to the present work. For the above algorithms, given a database $D$ comprising of a set of graphs $g=g_{1} ,g_{2} ,\ldots g_{n}$ and query $Q$ consisting of a set of graphs $q=q_{1} ,q_{2} ,\ldots q_{n}$, there are, in general, two kinds of graph search problems that we can define. By far the traditional and the most popular one is the subgraph query. Given a graph database $D$ and a subgraph query $Q$ with  a query graph $q$, the answer to $Q$ is the set of $\{g \| g\in D$ 
