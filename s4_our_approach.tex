...
To process a query over a graph database, the query is performed for each model graph in the $DAG$
In the $DAG$, processing subgraph matchings for isomorphism terminates as soon a non-isomorphic subgraph that is, a \textit{dead} state, is encountered. 
On termination, all the ancestors of the non-isomorphic subgraph as set to \textit{dead} state.  
