...
Messmer et al.\cite{messmer_bunke2000} proposed a method to search graphs which are induced subgraphs of a query graph from a graph database. 
...
Messmer called this data structure a network so here we refer to their method as the \textit{Network Method}. 
...
The \textit{Network Method} can be divided into two parts.
...
In the $DAG$, decomposed graph creation is directed by the original graph and on decomposition the directed graph is a \textit{child} graph and the directing graph is the \text{parent} graph. 
