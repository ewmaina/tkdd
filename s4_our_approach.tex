...
We have two main contributions. We propose a new search strategy for the database. We use this algorithm to implement fast and scalable graph database creation and query processing. Secondly, propose modifications to the $DAG$ data structure. B implementing these proposals we subgraph query search and subgraph query search on the modified $DAG$. that implements improvements in the core data structure to enable processing of both subgraphs and induced subgraphs, and propose a new fast and scalable search strategy for the database.
