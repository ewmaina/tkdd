...
By processing each model graph to completion, the \textit{Fast Network Algorithm} is more efficient as the state of larger common subgraphs is known for each model subgraph. When the shared larger subgraph is non-isomorphic to a query, the potential search space is reduced significantly.
