...
\texttt{(a)} Linear and \texttt{(b)} Linear-log plot of \textit{induced subgraph} query processing time against query graph size.

\texttt{(c)} Linear and \texttt{(d)} Linear-log plot of \textit{subgraph} query processing time for increasing size of graph query.
}
\label{fig:fig91}
\end{figure}
