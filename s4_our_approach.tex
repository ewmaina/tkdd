...
We have two main contributions. We propose a new search strategy for the database. We use this subgraph search algorithm to implement both database creation and query processing. Secondly, using the new search strategy we implement subgraph query search and subgraph query search on the modified $DAG$. that implements improvements in the core data structure to enable processing of both subgraphs and induced subgraphs, and propose a new fast and scalable search strategy for the database.
