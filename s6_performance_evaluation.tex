...
\caption{\textbf{Effect of Query Graph Size on Processing Time of Synthetic Graphs} 
 ...
 Linear plot \texttt{(a)} and Linear log plot \texttt{(b)} of induced subgraph query processing time against query graph vertex count.

 Linear plot \texttt{(c)} and \texttt{(d)} Linear-log plot of  subgraph query processing time for increasing size of graph query.
}
\label{fig:fig91}
\end{figure}
