...
Specifically, we describe in detail the application of the $DAG$ data structure to graph database building and a new novel search algorithm, \textit{ Fast Network Algorithm}. We contrast this with the \textit{Network Algorithm} proposed by Messmer et al. We use the same data strucure here as a basis to develop an database. \textit{New Network Algorithm}.
