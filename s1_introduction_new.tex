...
In contrast, \cite{messmer_bunke2000} builds the DAG is build offline by random decomposition of the database graphs, and storing of the subgraphs in the nodes of the DAG.   As a result the structure of the DAG network is not unique and varies randomly. However even for very large graphs the DAG forms a more balanced tree and so is efficient.  
During the search the DAG is not altered in any way. The DAG outputs all the subgraphs in the input graph that are also in the graph database. 
