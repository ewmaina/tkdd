...
\textt{(a)} Linear plot and Linear log plot \texttt{(b)} of induced subgraph query processing time against query graph vertex count.

\texttt{(c)} Linear plot and \texttt{(d)} Linear-log plot of  subgraph query processing time for increasing size of graph query.
}
\label{fig:fig91}
\end{figure}
