...
We have two main contributions. We propose a new search strategy for the graph database. We use this algorithm to implement fast and scalable graph database creation and query processing. Secondly, propose modifications to the $DAG$ data structure. By implementing these proposals we are able process both subgraph query search and induced subgraph query search on the modified $DAG$. 
