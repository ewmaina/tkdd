...
\caption{\textbb{Results of the Experiments on the Real World Graph Data Set:} \textbb{ Top left},Linear-linear plot of  query processing time for increasing size of induced graph query.The smallesst query graph contains 10 vertices while the largest query graph contains 60 vertices. The \textit{Fast Network Method} and the textit{Network Method} show almost identical processing times. The \textit{VF2} method takes more than 50s to process query graphs large than 10 vertices. 
